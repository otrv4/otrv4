...
1. [Offline Conversation Initialization](#offline-conversation-initialization)
   ...
   1. [Publishing pre-keys](#publishing-pre-keys)
   1. [Obtaining pre-keys](#obtaining-pre-keys)
...
2. Publishes the pre-key to the untrusted server.

**Alice:**

1. Request a pre-key from the untrusted server.
2. For each pre-key message received from the server:
    * Validates Bob's User Profile.
    * Picks the highest compatible version of OTR listed in Bob's profile.
...
The pre-key is an Identity message that is stored locally and published to an untrusted server.
